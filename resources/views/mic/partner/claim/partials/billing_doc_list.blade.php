...
<table class="table table-hover table-border mic-table">
  <tbody>
    ...
    <tr data-doc-id="{{ $doc['doc']->id }}" class="claim-billing-doc-item">
      ...
      <td class="doc-name text-color-primary">
        {!! MICUILayoutHelper::avatarImage($doc['doc']->creator, 48) !!}&nbsp;&nbsp;
        {{ $doc['doc']->creator->name }} : {{ $doc['doc']->file->name }}
      </td>
    ...
    <tr data-doc-id="{{ $reply_doc->id }}" class="claim-billing-doc-item billing-reply-doc">
      ...
      <td class="doc-name text-color-primary">
        {!! MICUILayoutHelper::avatarImage($reply_doc->creator, 48) !!} {{ $reply_doc->creator->name }} : 
        {{ $reply_doc->file->name }}
      </td>
